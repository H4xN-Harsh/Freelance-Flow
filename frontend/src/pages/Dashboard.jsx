    ...
    <div className='relative min-h-screen bg-bg-surface text-text-primary pt-28 pb-12 px-4'>
        ...
        <div className='max-w-6xl mx-auto space-y-8 relative z-10 w-full'>
            ...
            <section className='grid grid-cols-1 gap-4 sm:grid-cols-3'>{
                statCards.map((stat,i)=>(<div key={i}
                className='glass-panel p-6 rounded-2xl'>
                    <p className='text-sm font-mono uppercase tracking-wider text-text-muted'>{stat.label}</p>
                    <p className='text-2xl md:text-3xl font-light font-mono mt-2 text-text-primary'>{stat.Value}</p>
                </div>))
            }</section>
